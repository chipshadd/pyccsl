...
- Only the opt-in `usage-fable` field uses the network: a detached background process reads your Claude Code OAuth access token and fetches `api.anthropic.com/api/oauth/usage` at most every 5 minutes (see the [User Guide](pyccsl.md#cache-and-usage-fields))
